...
from tg_bot.modules.helper_funcs.groups_tracker import track_group


###                         dont forget to add register to register_handlers                         ###

def register_button_handler(application: Application) -> None:
    application.add_handler(CallbackQueryHandler(track_group(button_handler)))
...
def register_welcome_user(application: Application) -> None:
    application.add_handler(MessageHandler(filters.StatusUpdate.NEW_CHAT_MEMBERS, track_group(welcome_user)))

def register_handlers(application: Application) -> None:
    """Registers commands"""
    for cmd, func in COMMANDS.items():
        f = FILTERS.get(cmd)
        application.add_handler(CommandHandler(cmd, track_group(func), filters=f))
